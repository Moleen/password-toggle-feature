$(document).on('click','[data-toggle-eye]', function(){
    // Toggle between 'fa-eye' (open eye) and 'fa-eye-slash' (closed eye) icons
    $("[data-toggle='iconTogglePassword']").toggleClass('fa-eye');
    $("[data-toggle='iconTogglePassword']").toggleClass('fa-eye-slash');
    // Get the ID of the password input to be toggled
    var target = $(this).data('toggle-eye');
    // Reference the password input element by ID
    const passwordInput = $(`#${target}`);
    // Check the current input type; if it's 'password', change it to 'text', otherwise change it to 'password'
    const type = passwordInput.attr('type') === 'password' ? 'text' : 'password';
    // Set the new input type to display or hide the password
